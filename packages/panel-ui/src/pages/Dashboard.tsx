...
export default function Dashboard() {
    ...
    const stats = [
        { label: 'Status', value: <Badge variant={STATUS_VARIANT[status] ?? 'secondary'}>{status}</Badge>, icon: Activity },
        { label: 'PID', value: serverState?.pid ?? '—', icon: Hash },
        { label: 'Restarts', value: serverState?.restarts ?? 0, icon: RotateCcw },
        { label: 'Uptime', value: serverState?.startedAt ? formatUptime(serverState.startedAt) : '—', icon: Clock },
    ]
