import type { PlayerIdentifiers, PlayerUpdatePackage } from "@fxmanager/shared/types";
...
const PlayerEndpoints: RouteModule['handler'] = async (fastify, options) => {
  ...
  fastify.post('/deferrals', (request) => {
    ...
    return gm.playerDeferralChecks((body as { identifiers: PlayerIdentifiers }).identifiers);
  });

  fastify.post('/join', (request) => {
    const { body } = request;

    gm.playerJoin(body as { identifiers: PlayerIdentifiers, serverId: number, name: string });
    return { ack: true };
  });
